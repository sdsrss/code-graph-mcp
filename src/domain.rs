...
pub const INDEX_VERSION: i32 = 74; // v74 (2026-09-26): member calls bind the receiver's class. (1) `x.f()` bound every same-named method; when the source writes the receiver's class down (`this`/`self`, Python `super()` and constructor-assigned locals, a C++ local/parameter/in-class field's declared type, JS `new T()`, a TS `: T`) the call is stamped `{"q":"rtype"}` (`{"q":"super"}` for `super()`) and binds that class's method plus, except through `super()`, its overrides in subclasses of a uniquely named class; a class the project does not define (`std::string`, `AbortController`) binds nothing; a project class without the method resolves as an untyped member call. (2) Python `m.f()` through an absolute import of a module the project does not have (`click.echo()`) binds nothing. (3) TS `abstract class` was no class: no node, methods unqualified, no inherits edge. (4) A C/C++ `class`/`struct` without a body (forward declaration, `struct stat *st`) made a phantom node. (5) A JS nested function its factory returns in an object literal (`return { attemptUpgrade }`) is qualified `factory.name`, and a member call may reach it. (6) Editing a file restored every metadata-bound call into it (typed, `super()`, member) to all same-named nodes in the file; it now restores each to the method of the same qualified name, as a rebuild binds it (django: 66 and 179 extra edges after one edit on 0.157.0, now 0). leveldb inferred-tier call precision 798/844 -> 1355/1395 and recall 2118 -> 2620 of 3431 SCIP pairs. // v73 (2026-09-26): five call-graph errors found by scoring external corpora against SCIP (scripts/scip_oracle/corpora.sh: hono TS, express JS, flask Python, leveldb C++). (1) A call's SOURCE was bound by name to every same-named node in its file, so same-named definitions shared each other's callees (Rust cfg twins, Python `@overload` stubs, 37 nested `def index()` handlers in one flask test file); relations now carry their scope's start line (`ParsedRelation.source_line`) and the innermost name match containing it wins. (2) A gtest body's calls were sourced from `TEST_F`, a name no node has, so 228 of 229 leveldb TEST nodes had no outgoing call; the relation walk now names the scope `Suite.Case` as the node extractor does. (3) `class LEVELDB_EXPORT Slice {` parsed as a function `Slice` returning `class LEVELDB_EXPORT`, so every exported class became a one-line node named after the macro and its members free functions; C/C++ attribute macros between `class`/`struct` and the name are blanked (same byte length) before parsing. (4) A member call on an object (`words.push(x)`, `JSON.stringify(v)`, `s.clear()`, `ctx.get()`) bound free functions by bare name; C++/Python/JS member calls are stamped `{"q":"member"}` and never reach a `function` node. (5) A bare `f()` inside a C++ member (or gtest body) bound every same-file `f`; it now binds the caller's own `Class.f` when the file defines one (`DB::f()` is stamped `{"q":"scoped"}` and exempt, as is a constructor call). Old indexes keep all five errors until each file's content changes — hence the bump. // Older entries (v72 and down) live in CHANGELOG.md, which carries the same per-version narrative and its rebuild notices. Trimmed twice for the same reason: this one line is also a NODE in this project's own index, so every search over the repo carried it — 33,598 bytes at the first trim (2026-08-16 audit §四), 22,463 when it had grown back (2026-09-25). Keep the last two bumps here and move the rest when adding a third.
