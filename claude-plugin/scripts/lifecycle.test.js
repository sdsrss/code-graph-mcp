...
// --- JS-33: the reclaim must never hold the registry only in memory ---------

test('a failed restore leaves the registry on disk, and the next call puts it back', (t) => {
  // The finding itself. The version this replaces read the registry into the
  // process heap, wiped, and wrote it back: if that last write failed, the only
  // record of which repos carry a managed block was gone, and the function
  // returned success anyway. Rename-aside means the bytes are on disk under a
  // name the wipe cannot reach for the whole window, so the SAME failure is
  // recoverable — by the next run, without the user knowing anything happened.
  const homeDir = mkHome(t);
  const cacheDir = path.join(homeDir, '.cache', 'code-graph');
  const registryPath = path.join(cacheDir, 'adopted-projects.json');
  const stash = path.join(homeDir, '.cache', '.code-graph-adopted-projects.reclaim');
  const adopted = [{ cwd: '/some/project' }];
  writeJson(path.join(cacheDir, 'bin', 'marker.json'), { v: 1 });
  writeJson(registryPath, adopted);
  ...
  const out = execFileSync(process.execPath, ['-e', `
    ...
    // Fail only the RESTORE (stash -> registry); the aside-move must succeed.
    fs.renameSync = (a, b) => {
      if (String(b).endsWith('adopted-projects.json')) throw new Error('injected restore failure');
      return orig(a, b);
    };
    const { removeCacheResidue } = require(${JSON.stringify(lifecyclePath)});
    process.stdout.write(JSON.stringify(removeCacheResidue()));
  ...
  assert.equal(out, 'true', 'the cache WAS reclaimed, which is what the return value reports');
  assert.equal(fs.existsSync(cacheDir), false, 'and the cache dir really is gone');
  assert.ok(fs.existsSync(stash), 'the registry bytes must still exist somewhere on disk');
  assert.deepEqual(
    JSON.parse(fs.readFileSync(stash, 'utf8')), adopted,
    'and they must be the registry, unchanged',
  );

  // Recovery is automatic on the next reclaim — no user action, no lost record.
  execFileSync(process.execPath, ['-e', `
    require(${JSON.stringify(lifecyclePath)}).removeCacheResidue();
  `], { env: { ...process.env, HOME: homeDir, USERPROFILE: homeDir } });
  assert.deepEqual(
    JSON.parse(fs.readFileSync(registryPath, 'utf8')), adopted,
    'the next call restores the stash before deciding anything',
  );
  assert.equal(fs.existsSync(stash), false, 'and clears the stash it consumed');
});

test('a stash left by a crashed run is restored before the wipe decision', (t) => {
  // Same window, entered from the other side: a run that died between the
  // rename and the restore leaves a stash and no registry. Without recovery the
  // next call reads "no registry, nothing to preserve" and the record is lost
  // for good on a path that looks completely normal.
  const homeDir = mkHome(t);
  const cacheDir = path.join(homeDir, '.cache', 'code-graph');
  const registryPath = path.join(cacheDir, 'adopted-projects.json');
  const stash = path.join(homeDir, '.cache', '.code-graph-adopted-projects.reclaim');
  const adopted = [{ cwd: '/crashed/mid/reclaim' }];
  writeJson(path.join(cacheDir, 'bin', 'marker.json'), { v: 1 });
  writeJson(stash, adopted);
  assert.equal(fs.existsSync(registryPath), false, 'precondition: the crash left no registry');
  ...
  assert.deepEqual(
    JSON.parse(fs.readFileSync(registryPath, 'utf8')), adopted,
    'the orphaned stash must come back as the registry',
  );
  assert.deepEqual(
    fs.readdirSync(cacheDir), ['adopted-projects.json'],
    'and the reclaim still did its job on everything else',
  );
});

test('a live peer holding the reclaim lock makes this call stand down', (t) => {
  // The concurrency hole that made the FIRST attempt at this fix worse than the
  // bug: two reclaims interleave, the second reads "no registry" (the first has
  // it renamed aside) and wipes — and its wipe can land after the first has
  // restored, taking the restored file with it. Standing down is the only safe
  // answer, and it has to be decided under the lock, before the
  // nothing-to-preserve question is even asked.
  const homeDir = mkHome(t);
  const cacheDir = path.join(homeDir, '.cache', 'code-graph');
  const lockPath = path.join(homeDir, '.cache', '.code-graph-reclaim.lock');
  writeJson(path.join(cacheDir, 'bin', 'marker.json'), { v: 1 });
  // A live holder: this test process is alive by definition, so the lock cannot
  ...
  assert.equal(out, 'false', 'a peer holds the lock, so this call removed nothing');
  assert.ok(fs.existsSync(path.join(cacheDir, 'bin', 'marker.json')), 'and wiped nothing');

  // Negative control: with the lock gone the very same call proceeds, so the
  // assertion above is about the LOCK and not about some unrelated refusal.
  fs.unlinkSync(lockPath);
  const after = execFileSync(process.execPath, ['-e', `
    const { removeCacheResidue } = require(${JSON.stringify(lifecyclePath)});
    process.stdout.write(JSON.stringify(removeCacheResidue()));
  `], { env: { ...process.env, HOME: homeDir, USERPROFILE: homeDir } }).toString();
